...
async function startServer() {
  try {
    console.error("Starting Google Drive MCP server with access token authentication");
    
    // Validate access token at startup
    const isValid = await validateAccessToken();
    if (!isValid) {
      console.error("Failed to validate access token. Please check your GOOGLE_DRIVE_ACCESS_TOKEN environment variable.");
      process.exit(1);
    }
    
    console.error("Access token validated successfully");
